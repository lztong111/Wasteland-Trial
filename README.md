...
- 消耗体力的方向闪避与短暂无敌时间
- 三段轻攻击连击、蓄力重击与远程投射物（真实时间驱动，非帧数）
- 敌人追击、攻击前摇、受伤、死亡与经验奖励
- “清除第一波守卫 → 开启补给箱 → 击退增援 → 激活神龛”的完整关卡目标与胜利结算
- 结算后可进入下一轮，敌人生命、伤害和经验奖励随轮次提高
...
开发环境中，如果附近没有可交互物，按 `E` 会增加 10 点测试经验；生产构建不包含该行为。
...
- `GameProgressManager` 管理任务顺序、胜利状态与世界交互进度

## 存档

- 存储：浏览器 `localStorage`
- 当前键名 / 版本：`gm.save.v3` / `3`，由 `gameConfig.save` 统一管理
- 第三版同时保存 RPG 数据、两波守卫、宝箱、神龛与胜利确认状态
- 可读取第一、二版存档并保留成长数据；由于关卡流程变化，旧任务进度会重置
